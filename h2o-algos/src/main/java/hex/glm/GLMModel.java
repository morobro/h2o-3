      ...
      if (_family.equals(Family.negativebinomial)) {
        ...
        x.w = _var_power==1?(y*oneOxmuSquare*diffOneSquare-(y*oneOxmu-1)*linkInvDeriv2(x.mu)):(_var_power==2?
                ((oneOxmu-y*xmuPowMP)*linkInvDeriv2(x.mu)+
                        (y*_var_power*Math.pow(x.mu, -_var_power-1)-oneOxmuSquare)*diffOneSquare)
                :(_var_power*y*Math.pow(x.mu, -_var_power-1)+_oneMinusVarPower*xmuPowMP)*diffOneSquare-
                (y*Math.pow(x.mu, 1-_var_power)-y*xmuPowMP)*linkInvDeriv2(x.mu));
        x.z = etaOff-_var_power==1?(1-y*oneOxmu)*linkInvDeriv(x.mu):
                (_var_power==2?(oneOxmu-y*Math.pow(x.mu, -_var_power)):
                        (Math.pow(x.mu, _oneMinusVarPower)-y*Math.pow(x.mu, -_var_power)))/x.w;
        x.w *= w;
      } else {
